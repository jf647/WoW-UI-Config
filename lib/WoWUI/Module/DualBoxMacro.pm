...
sub BUILD
{
    ...
    # build filter groups
    my $fgs =
      WoWUI::FilterGroups->new( $config->{filtergroups}, $config->{buttons}, );
...
sub augment_perchar
{
    ...
    my $ntl =
      WoWUI::Modules->instance->module_get('WoWUI::Module::NADTTrustList');
    ...
    for my $barname ( keys %{ $config->{bars} } ) {
        ...
        for my $buttonpos (
            sort { $a <=> $b }
            keys %{ $config->{bars}->{$barname}->{buttons} }
          )
        {
            my $button;
            my $match = $self->find_button(
                $char,
                $f,
                $config->{bars}->{$barname}->{buttons}->{$buttonpos}
                  ->{buttontype},
        ...
        $bar->{button_i} =
          join( ';', grep { defined $_ } @{ $bar->{buttons} } );
        $bar->{autoHide} =
          exists $config->{bars}->{$barname}->{autohide} ? 1 : 0;
...
sub find_button
{
    ...
    for my $button ( $candidates->members ) {
          ...
          )
        {
            if ($found) {
                croak
                  "buttontype '$buttontype' matched two buttons ($match and $button)";
            }
            $found = 1;
            $match = $button;
        }
...
sub build_button
{
    ...
    if ( exists $buttoncfg->{macro} ) {
        ...
        $button->{name}  = $buttoncfg->{name}
          || croak "no name for macro '$buttonname'";
        return $button;
    }
        ...
        if ( exists $buttoncfg->{ooc} ) {
            if ( exists $data->{seq} && 1 == $data->{seq} ) {
                croak
                  "ooc alternative is incompatible with a spell sequence in $buttoncfg->{spell}";
            }
            $data->{ooc}       = 1;
