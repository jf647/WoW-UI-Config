...
has [qw|global globalpc perchar|] =>
...
use WoWUI::Util
  qw|expand_path load_layered perchar_sv sv tempfile tt log tempdir|;
...
sub BUILD
{
    ...
    my $name = lc( ref $self );
    $name =~ s/^wowui::module:://x;
    $self->name($name);
    $self->config(
        load_layered( "$name.yaml", '$ADDONCONFDIR', '$PRIVADDONCONFDIR' ) );

    if ( exists $self->config->{modoptions} ) {
        $self->set_modoptions(
            { modoptions => { $self->name => $self->config->{modoptions} } } );
    }
...
sub register_char
{
    ...
    return
      if (
        exists $self->globaldata->{charlist}->{ $char->realm->name }
        ->{ $char->name } );
    $self->globaldata->{charlist}->{ $char->realm->name }->{ $char->name } =
      $value || 1;
...
sub write_global
{
    ...
    for my $template ( keys %{ $config->{templates}->{global} } ) {
        my $infname =
          expand_path( $config->{templates}->{global}->{$template}->{input} );
        my $outfname =
          expand_path( $config->{templates}->{global}->{$template}->{output},
        ...
        if ( $config->{templates}->{global}->{$template}->{pretty_print} ) {
            my $pp_outfname = expand_path(
                ...
                account => $self->player->account, );
            $self->pretty_print_lua( $outfname, $pp_outfname,
                $config->{templates}->{global}->{$template}->{pp_varname} );
        }
    }
...
sub write_perchar
{
    ...
    for my $template ( keys %{ $config->{templates}->{perchar} } ) {
        my $infname =
          expand_path( $config->{templates}->{perchar}->{$template}->{input} );
        ...
        if ( $config->{templates}->{perchar}->{$template}->{pretty_print} ) {
            ...
            $self->pretty_print_lua( $outfname, $pp_outfname,
                $config->{templates}->{perchar}->{$template}->{pp_varname} );
        }
    }
    ...
    if ( exists $config->{copy} ) {
        for my $copy ( keys %{ $config->{copy}->{perchar} } ) {
            my $src =
              expand_path( $config->{copy}->{perchar}->{$copy}->{from} );
...
sub pretty_print_lua
{
    ...
    my @cmd = ( $gcfg->{pretty_print}->{lua}, $tempfname1, );
